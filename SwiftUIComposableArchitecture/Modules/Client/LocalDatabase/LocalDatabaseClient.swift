...
extension LocalDatabaseClient {
    static func mock(migrate: @escaping () -> Effect<Void, Error> = { fatalError("Unmocked") },
                     ...
                     deleteFavoriteCard: @escaping (Card) -> Effect<[Card], Never> = { _ in fatalError("Unmocked") }) -> Self {
        Self(
            migrate: migrate,
    ...
    static func mockPreview(migrate: @escaping () -> Effect<Void, Error> = { .result { .success(()) } },
                            ...
                            deleteFavoriteCard: @escaping (Card) -> Effect<[Card], Never> = { _ in .init(value: []) }) -> Self {
        Self(
            migrate: migrate,
            fetchFavoriteCards: fetchFavoriteCards,
            insertFavoriteCard: insertFavoriteCard,
